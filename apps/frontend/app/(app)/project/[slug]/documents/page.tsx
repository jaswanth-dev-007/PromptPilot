...
export default function ProjectDocuments({ params }: { params: { slug: string } }) {
  ...
  async function handleGenerate(stepId: string) {
    ...
    setError(null)
    try {
      const res = await api.post<{ document: { status: string; content: string } }>('/pipeline/generate', {
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <Link href={`/project/${params.slug}`} style={{
            ...
            {ARTIFACTS.map(a => {
              ...
              return (
                <div key={a.id} style={{
                  ...
                  borderRadius: '12px', border: result ? '1px solid #A7F3D0' : '1px solid #E2E8F0',
                  backgroundColor: result ? '#F0FDF4' : '#F8FAFC',
                }}>
                  <span style={{ fontSize: '1.5rem' }}>{a.icon}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.875rem', fontWeight: 600, color: '#111827' }}>{a.label}</div>
                    <div style={{ fontSize: '0.75rem', color: result?.status === 'GENERATED' ? '#059669' : '#9CA3AF' }}>
                      {result?.status === 'GENERATED' ? 'Generated' : generating === a.id ? 'Generating...' : 'Not generated'}
                    </div>
                  </div>
                  {!result && (
                    <button
                      ...
                      style={{
                        padding: '4px 12px', borderRadius: '6px', backgroundColor: generating === a.id ? '#9CA3AF' : '#4F46E5',
                        color: '#FFF', fontSize: '0.75rem', fontWeight: 600, border: 'none', cursor: generating === a.id ? 'not-allowed' : 'pointer',
