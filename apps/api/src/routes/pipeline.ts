...
const SIMPLE_TEMPLATE = {
  ...
  category: 'generation',
  systemPrompt:
    'You are an expert software engineer. Generate a high-quality, well-structured engineering document based on the provided context and instructions.',
    ...
    async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      try {
        ...
        if (!projectId || !stepId) {
          res.status(400).json({
            success: false,
            error: { code: 'VALIDATION_ERROR', message: 'projectId and stepId are required' },
          })
          return
        }
        ...
        for (const stepId of steps) {
          try {
            ...
            results.push({
              stepId,
              status: 'completed',
              tokens: result.conversation?.totalInputTokens,
            })
          } catch {
